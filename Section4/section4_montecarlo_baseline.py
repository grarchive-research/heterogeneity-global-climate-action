...
Description
-----------
Monte Carlo pipeline for the agent-based model (Section 4), equal
country weights: 2N=194 countries on a fully-connected network. Nested
double Monte Carlo — 100 draws of the idiosyncratic error term (inner
loop) x 194 starting countries, one per possible first mover (outer
loop) — averaged to produce the mean and standard-deviation band of
participation and net emissions. Produces the published Fig.
Comparison_evolution_discrete.
...
def MonteCarlo_194(N, T, MC_runs):
    ...
    for k in range(N):  # Iterate over each country as the starting point
        ...
        for mc in range(MC_runs):  # Monte Carlo loop for error term variations
            ...
            E, X, pi, VE, nC, y, Z, ZZ, A, C, D = GRT_AMBnetEJ(T=T, betan=1.1, betae=0.18, betad=0, N=N, a=0, mu=9.77, start_idx=k)
...
mean_E, mean_VE, mean_X, mean_nC, std_E, std_X, std_VE, std_nC = MonteCarlo_194(194, 500, MC_runs=100)
...
ax1.fill_between(range(T), mean_X - std_X, mean_X + std_X, color=color, alpha=0.2)
...
ax2.fill_between(range(T), mean_E - std_E, mean_E + std_E, color=color, alpha=0.2)
...
plt.savefig('MonteCarlo_200.png', dpi=1200, format='png', bbox_inches='tight')
plt.show()
